    ...
    public static class LinqTasks {
        ...
        public static int Task13(int[] arr) {
            ...
            if (tmp.ToList().Count != 1)
                throw new Exception(JsonConvert.SerializeObject(tmp, Formatting.Indented));
        ...
        public static IEnumerable<object> Task15() {
            IEnumerable<object> result = Emps
                .Where(x => x.Job.Contains("A")) //jeżeli to by bylo 'a' zamiast 'A', to byśmy otrzymali w result "Frontend programmer"
                .GroupBy(e => e.Job)
                .Select(x => new {
                    Praca = x.Key,
                    LiczbaPracownikow = x.Count()
                })
                .Where(x => x.LiczbaPracownikow > 2)
    ...
    public static class CustomExtensionMethods {
        ...
        public static IEnumerable<Emp> GetEmpsWithSubordinates(this IEnumerable<Emp> emps) {
            var result = emps.Where(e =>
                    emps.Where(e => e.Mgr is not null)
                    .Any(e2 => e2.Mgr.Empno == e.Empno))
